 ...
 * @link     pbgroupeu.wordpress.com
 */
class VoiceDrillTest extends BaseEtlTddUnit
{
    /**
    ...
    public function testLoadContainerData(): void
    {
        ...
        $this->assertStringContainsString(VoiceModel::class, $response['job']);
        $this->assertStringContainsString('OK', $response['status']);

        $stmt = $pdoEtl->prepare(
            'SELECT COUNT(id) FROM voice_etl'
        );
        $stmt->execute();
